...
export const createTimelineFromAnalysis = (result: any, isRTL: boolean = false) => {
  ...
  if (result.modules.biometrics.matches.length > 0) {
    ...
    events.push({
      ...
      type: 'detection',
      title: isRTL ? 'تحديد الهوية البيومترية' : 'Biometric Identification',
      description: `Suspect identified as ${match.info.name} with ${Math.round(match.confidence * 100)}% confidence`,
      color: 'bg-amber-500'
    });
  }
  ...
  if (result.modules.GPS) {
    events.push({
      ...
      type: 'location',
      title: isRTL ? 'تثليث موقع GPS' : 'GPS Triangulation',
      description: `Location pinpointed with ${Math.round(result.modules.GPS.confidence * 100)}% accuracy`,
  ...
  if (result.modules.cctv_retrieval.cctv_nodes.length > 0) {
    events.push({
      ...
      type: 'cctv',
      title: isRTL ? 'شبكة المراقبة' : 'Surveillance Network Active',
      description: `${result.modules.cctv_retrieval.cctv_nodes.length} cameras within ${result.modules.cctv_retrieval.meta.search_radius} radius identified`,
      color: 'bg-primary'
    });
  }
  ...
  if (hasThreats) {
    events.push({
      ...
      type: 'threat',
      title: isRTL ? 'كشف التهديد' : 'Threat Detected',
      description: `Suspicious objects identified: ${result.modules.object_detection.detections.filter((d: any) => d.threat_tag).map((d: any) => d.label).join(', ')}`,
      color: 'bg-red-500'
    });
  }
